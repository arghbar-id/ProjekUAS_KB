...
def get_signatures_and_labels(main_path=None):
    # Jika main_path tidak diberikan, gunakan path relatif ke folder 'dataset'
    if main_path is None:
        main_path = os.path.join(os.path.dirname(__file__), 'dataset')
    
    print(f"Checking path: {main_path}")
    
    # Validasi apakah folder dataset ada
    if not os.path.exists(main_path):
        raise FileNotFoundError(f"Path {main_path} does not exist!")
